...
{
  float u, t, h;
  thrust_math(float _u, float _t, float _h) :
      u(_u), t(_t), h(_h) {};
...
int main()
{
    ...
    thrust::transform(thrust::device, deviceVector.begin() + 1, deviceVector.end(), deviceVector.begin(), deviceVector.begin(), thrust_math(1.1, 0.9, 1.4));
